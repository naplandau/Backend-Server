...
async fn main() -> std::io::Result<()> {
    ...
    let redisFac = RedisFactory::connect(config::CONFIG.redis_url.to_owned())
        .await
        .expect("");
    let rabbitFac = RabbitFactory::get_pool(config::CONFIG.rabbit_url.to_owned())
        .await
        .expect("");
    let mut consumer = RabbitServer::create_consumer(&rabbitFac, "ha_qu_test", "test")
        .await
        .unwrap();
