...
use mongodb::{options::FindOptions, Client, Cursor};
use rayon::prelude::*;
use std::sync::Mutex;
use uuid::Uuid;
...
pub async fn test_mongo(data: Data<Mutex<Client>>) -> impl Responder {
    let client = data.lock().unwrap();
    let mut cursor = db_utils::find_all(&client, "users").await.unwrap();
